...
public class ClassPathScanningCandidateComponentProvider implements EnvironmentCapable, ResourceLoaderAware {
	 ...
	 * @Controller 注解的 Bean ，以及 Java EE 6 的@Managed Bean 和 JSR - 330 的＠Named 注解
	 */
	...
	protected void registerDefaultFilters() {
		...
		try {
			//向要包含的过滤规则添加 Java EE 6 的 @ManagedBean 注解
		...
		try {
			...
			for (String type : types) {
				...
				if (isCandidateComponent(metadataReader)) {
					//通过汇编(ASM)读取资源字节码中的 Bean 定义的元信息
	...
	protected boolean isCandidateComponent(MetadataReader metadataReader) throws IOException {
		...
		//如果读取的类的注解在包含的注解过滤规则中，则返回 ture
		for (TypeFilter tf : this.includeFilters) {
			if (tf.match(metadataReader, getMetadataReaderFactory())) {
				return isConditionMatch(metadataReader);
			}
		}
		//如果读取的类的注解既不在排除规则中 ， 也不在包含规则中，则返回 false
		return false;
	}
