...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected void doRegisterBeanDefinitions(Element root) {
		...
		//具体的解析过程由 BeanDefinitionParser、Delegate 实现
	...
	/**
	 * 创建 BeanDefinitionParserDelegate ， 用于完成真正的角种斤过程
	 * */
	protected BeanDefinitionParserDelegate createDelegate(
			XmlReaderContext readerContext, Element root, @Nullable BeanDefinitionParserDelegate parentDelegate) {

		BeanDefinitionParserDelegate delegate = new BeanDefinitionParserDelegate(readerContext);
		//BeanDefinitionParseDelegate 初始化 Document 根元素
	...
	protected void parseBeanDefinitions(Element root, BeanDefinitionParserDelegate delegate) {
		...
		if (delegate.isDefaultNamespace(root)) {
			//获取 Bean 定义 的文档对象根元素的所有子节点
			NodeList nl = root.getChildNodes();
			for (int i = 0; i < nl.getLength(); i++) {
				Node node = nl.item(i);
				//Bean 定义的文档的元素节点使用的是 Spring 默认的 XML 命名空间
	 ...
	 * 解析 <import> 导入元素 ， 从给定的导入路径加载 Bean 资源到 Spring IoC 容器中
	 */
	...
	protected void processAliasRegistration(Element ele) {
		...
		boolean valid = true;
		//获取<alias>别名元素的 name 属性值为空
		if (!StringUtils.hasText(name)) {
			getReaderContext().error("Name must not be empty", ele);
			valid = false;
		}
		//获取<alias>别名元素的 alias 属性值为空
	 ...
	 * 解析 Bean 资源文档对象 的普通元素
	 */
	protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
		//解析 <bean> 元素
		BeanDefinitionHolder bdHolder = delegate.parseBeanDefinitionElement(ele);
		//BeanDefinitionHolder 是对 BeanDefinition 的封装 ，即 Bean 定义的封装类
		//对文档对象中<bean>元素 的解析 由 BeanDefinitionParserDelegate 实现
		if (bdHolder != null) {
			bdHolder = delegate.decorateBeanDefinitionIfRequired(ele, bdHolder);
			try {
				// Register the final decorated instance.
				//向 Spring IoC 容器注册解析得到的 Bean 定义 ， 这是 Bean 定义 向 IoC 容器注册的入口
			...
			//在完成 向 Spring IoC 容器注册解析得到的 Bean 定义之后 ， 发送注册事件
